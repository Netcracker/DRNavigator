"""Functions and parameters that are used in server site"""
import base64
import logging
import os
import sys
...
if SM_CONFIG_FILE != "":
    with open(SM_CONFIG_FILE) as file:
        try:
            SM_CONFIG = yaml.safe_load(file)
...
def get_all_services(sm_dict):
    ...
    return list(sm_dict["services"].keys())
...
def get_token(api_watch=False):
    ...
    if not api_watch:
        ...
        try:
            service_account = client.CoreV1Api().read_namespaced_service_account("sm-auth-sa", namespace)
            secret_name = [s for s in service_account.secrets if 'token' in s.name][0].name
            btoken = client.CoreV1Api().read_namespaced_secret(name=secret_name, namespace=namespace).data['token']
        ...
        while True:
            ...
            for event in w.stream(client.CoreV1Api().list_namespaced_service_account,
                                  namespace,
                                  timeout_seconds=30):
                if event['object'].metadata.name == "sm-auth-sa":
                    if event['type'] in ["ADDED", "MODIFIED"]:
                        ...
                        btoken = client.CoreV1Api().read_namespaced_secret(
