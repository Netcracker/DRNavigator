...
def sitemanager_post():
    ...
    if data["procedure"] == "status":
        output = dict()
        module = import_module(sm_dict["services"][run_service]["module"])
        output[run_service] = module.get_status(sm_dict["services"][run_service], **data)
        return json_response(200, output)

    logging.info(f"Start process service: {run_service}")
    resp = utils.send_post(url=sm_dict["services"][run_service]['parameters']["serviceEndpoint"],
                           mode=data["procedure"],
                           no_wait=no_wait)
    if resp.get("bad_response") or resp.get("fatal"):
        return json_response(500, {"message": f"Procedure {data['procedure']} failed",
                                   "service-failed": run_service,
